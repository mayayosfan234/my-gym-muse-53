...
import { ArrowLeft, ChevronDown, Flame, GripVertical, Link2, Link2Off, Plus, Save, Search, SlidersHorizontal, Trash2, X } from "lucide-react";
...
export const Route = createFileRoute("/programs/$programId/$dayId")({
  head: () => ({ meta: [{ title: "Day Builder — GymTrack" }] }),
...
function DayBuilder() {
  ...
  if (!program || (!isNew && !existing)) return <AppShell title="Day not found"><p className="surface-card p-5 text-muted-foreground">This training day no longer exists.</p></AppShell>;

  const labels = computeSupersetLabels(draft.items);

  const patchItem = (id: string, patch: Partial<WorkoutItem>) => setDraft((current) => ({ ...current, items: current.items.map((item) => item.id === id ? { ...item, ...patch } : item) }));
  ...
  const pickerExercises = exercises.filter(
    (exercise) =>
      (pickerGroup === "All" || exercise.muscleGroup === pickerGroup) &&
      (pickerEquipment === "All" || exercise.equipment === pickerEquipment) &&
  ...
  return (
    <AppShell title={isNew ? "New training day" : draft.name} subtitle={`${draft.items.length} exercises`} action={<Link to="/programs/$programId" params={{ programId }} aria-label="Back to program" className="grid h-11 w-11 place-items-center rounded-xl bg-secondary active:scale-95"><ArrowLeft className="h-5 w-5" /></Link>}>
      <section className="surface-card p-4 sm:p-5">
        <label className="section-kicker" htmlFor="day-name">Day name</label>
        <input id="day-name" data-testid="input-day-name" value={draft.name} onChange={(event) => setDraft({ ...draft, name: event.target.value })} className={`${field} mt-2 font-display text-lg font-semibold`} placeholder="e.g. Monday · Lower" />
        <label className="section-kicker mt-4 block" htmlFor="day-notes">Notes</label>
        <textarea id="day-notes" rows={2} value={draft.notes} onChange={(event) => setDraft({ ...draft, notes: event.target.value })} className={`${field} mt-2`} placeholder="A cue, a focus, a reason..." />
      </section>

      <div className="mt-7 flex items-end justify-between"><div><p className="section-kicker">Exercise order</p><h2 className="mt-1 font-display text-2xl font-semibold">The work</h2></div><span className="text-xs text-muted-foreground">Drag to reorder</span></div>
      <DndContext sensors={sensors} collisionDetection={closestCenter} onDragEnd={onDragEnd}>
        <SortableContext items={draft.items.map((item) => item.id)} strategy={verticalListSortingStrategy}>
          <div className="mt-4 space-y-3">{draft.items.map((item, index) => {
      ...
      <button type="button" data-testid="button-add-exercise-to-day" onClick={() => setPicker(true)} className="mt-4 flex h-14 w-full items-center justify-center gap-2 rounded-xl border border-dashed border-primary/40 bg-primary/5 font-semibold text-primary active:scale-[0.98]"><Plus className="h-5 w-5" /> Add exercise</button>
      <button type="button" data-testid="button-save-day" onClick={save} className="mt-3 flex h-14 w-full items-center justify-center gap-2 rounded-xl bg-primary text-base font-semibold text-primary-foreground active:scale-[0.98]"><Save className="h-5 w-5" /> Save training day</button>
      {!isNew ? <button type="button" onClick={() => window.confirm(`Delete ${draft.name}? Saved history will stay.`) && (deleteWorkout(draft.id), navigate({ to: "/programs/$programId", params: { programId } }))} className="mt-3 flex h-14 w-full items-center justify-center gap-2 rounded-xl bg-secondary font-semibold text-destructive active:scale-[0.98]"><Trash2 className="h-5 w-5" /> Delete day</button> : null}
      ...
      {picker ? (
        ...
        >
          <div
            className="max-h-[82vh] overflow-y-auto rounded-t-3xl border-t border-border bg-card p-5"
            onClick={(event) => event.stopPropagation()}
          >
            <div className="mb-4 flex items-center justify-between">
              <div>
                <p className="section-kicker">Exercise library</p>
                <h2 className="font-display text-xl font-semibold">Add to this day</h2>
              </div>
              <button
                type="button"
                onClick={() => setPicker(false)}
            ...
            <div className="mt-4 space-y-2">
              {pickerExercises.map((exercise) => (
                <button
                  ...
                  className="flex w-full items-center gap-3 rounded-xl bg-secondary p-4 text-left active:scale-[0.99]"
                >
            ...
            {creatingExercise ? (
              <div className="mt-4 rounded-2xl border border-primary/20 bg-primary/5 p-4">
                <label className="section-kicker" htmlFor="new-picker-exercise">
                  New exercise
                </label>
...
function SortableItem({
  ...
  return (
    <article ref={setNodeRef} style={{ transform: CSS.Transform.toString(transform), transition }} className={`surface-card p-4 ${isDragging ? "relative z-10 opacity-75 shadow-lg" : ""} ${supersetLabel ? "border-l-4 border-l-primary/60" : ""}`}>
      <div className="flex items-center gap-2">
        <button type="button" {...attributes} {...listeners} aria-label={`Reorder ${name}`} className="grid h-9 w-9 shrink-0 touch-none place-items-center rounded-lg bg-secondary text-muted-foreground"><GripVertical className="h-4 w-4" /></button>
        <div className="min-w-0 flex-1">
          <div className="flex items-center gap-2">
        ...
        <button type="button" onClick={onRemove} aria-label={`Remove ${name}`} className="grid h-9 w-9 shrink-0 place-items-center rounded-lg bg-secondary text-destructive"><X className="h-4 w-4" /></button>
      </div>
      ...
      {canSuperset ? (
        <button
          ...
          className={`mt-3 flex w-full items-center justify-center gap-1.5 rounded-lg py-2 text-xs font-semibold ${supersetActive ? "bg-primary/15 text-primary" : "bg-secondary text-muted-foreground"}`}
        >
          {supersetActive ? <Link2Off className="h-3.5 w-3.5" /> : <Link2 className="h-3.5 w-3.5" />}
          {supersetActive ? "Break superset with above" : "Superset with exercise above"}
        </button>
      ) : null}

      <div className="mt-3 grid grid-cols-2 gap-3">
        <Stepper label="Sets" value={item.sets} min={1} onChange={(value) => onPatch(item.id, { sets: value })} />
        <Stepper label="Rest" value={item.rest} step={15} suffix="s" onChange={(value) => onPatch(item.id, { rest: value })} />
      </div>

      <div className="mt-3">
        <div className="mb-1.5 flex items-center justify-between">
          <p className="text-[11px] font-medium uppercase tracking-wide text-muted-foreground">Reps</p>
          <div className="flex rounded-lg bg-secondary p-0.5 text-[11px] font-semibold">
            <button type="button" onClick={() => setRepType("fixed")} className={`rounded-md px-2.5 py-1 ${!isRange ? "bg-primary text-primary-foreground" : "text-muted-foreground"}`}>Fixed</button>
            <button type="button" onClick={() => setRepType("range")} className={`rounded-md px-2.5 py-1 ${isRange ? "bg-primary text-primary-foreground" : "text-muted-foreground"}`}>Range</button>
          </div>
        </div>
        {isRange ? (
          <div className="grid grid-cols-2 gap-3">
            <Stepper label="Min reps" value={item.repMin ?? item.reps} min={0} onChange={(value) => onPatch(item.id, { repMin: value })} />
            <Stepper label="Max reps" value={item.repMax ?? item.reps} min={0} onChange={(value) => onPatch(item.id, { repMax: value })} />
          </div>
        ) : (
          <Stepper label="Target reps" value={item.reps} min={0} onChange={(value) => onPatch(item.id, { reps: value })} />
        )}
      </div>

      <div className="mt-3">
        <Stepper label="Working weight" value={item.weight} step={2.5} suffix="kg" onChange={(value) => onPatch(item.id, { weight: value })} />
      </div>

      <div className="mt-3 rounded-xl border border-border bg-secondary/40 p-3">
        <p className="mb-2 text-xs font-semibold text-muted-foreground">Weight and reps for each set</p>
        <div className="space-y-2">
          {workingSets.slice(0, item.sets).map((set, index) => (
            <div key={set.id} className="space-y-2">
              <div className="grid grid-cols-[2rem_1fr_1fr] items-end gap-2">
                <span className="pb-3 text-center text-xs font-semibold text-muted-foreground">{index + 1}</span>
                <Stepper label="kg" value={set.weight} step={2.5} onChange={(weight) => onPatch(item.id, { workingSets: workingSets.map((current, i) => i === index ? { ...current, weight } : current) })} />
                <Stepper label="reps" value={set.reps} min={0} onChange={(reps) => onPatch(item.id, { workingSets: workingSets.map((current, i) => i === index ? { ...current, reps } : current) })} />
              </div>
      ...
      <input className={`${field} mt-3`} value={item.notes} onChange={(event) => onPatch(item.id, { notes: event.target.value })} placeholder="A note for this exercise (e.g. slow eccentric)" />

      {/* Warm-up sets */}
      {warmups.length > 0 ? (
        <div className="mt-3 rounded-xl border border-dashed border-border bg-secondary/50 p-3">
          <div className="mb-2 flex items-center gap-1.5 text-xs font-semibold text-muted-foreground"><Flame className="h-3.5 w-3.5 text-primary" /> Warm-up sets</div>
          <div className="space-y-2">
            {warmups.map((w, wi) => (
              <div key={w.id} className="flex items-center gap-2">
                <span className="w-5 shrink-0 text-center text-xs font-semibold text-muted-foreground">{wi + 1}</span>
                <div className="grid flex-1 grid-cols-2 gap-2">
                  <Stepper label="kg" value={w.weight} step={2.5} onChange={(value) => patchWarmup(w.id, { weight: value })} />
                  <Stepper label="reps" value={w.reps} min={0} onChange={(value) => patchWarmup(w.id, { reps: value })} />
                </div>
                <button type="button" aria-label="Remove warm-up set" onClick={() => onPatch(item.id, { warmups: warmups.filter((x) => x.id !== w.id) })} className="grid h-9 w-9 shrink-0 place-items-center rounded-lg bg-background text-destructive"><X className="h-4 w-4" /></button>
              </div>
            ))}
          </div>
          <button type="button" onClick={() => onPatch(item.id, { warmups: [...warmups, emptyWarmup()] })} className="mt-2 w-full rounded-lg bg-background py-2 text-xs font-semibold text-primary">Add warm-up set</button>
        </div>
      ) : (
        <button type="button" onClick={() => onPatch(item.id, { warmups: [emptyWarmup()] })} className="mt-3 flex w-full items-center justify-center gap-1.5 rounded-lg bg-secondary py-2 text-xs font-semibold text-muted-foreground"><Flame className="h-3.5 w-3.5" /> Add warm-up sets</button>
      )}
      ...
      <button type="button" onClick={() => setAdvanced((v) => !v)} className="mt-3 flex w-full items-center justify-between rounded-lg px-1 py-1.5 text-xs font-semibold text-muted-foreground">
        <span className="flex items-center gap-1.5"><SlidersHorizontal className="h-3.5 w-3.5" /> Advanced (tempo · RIR · RPE)</span>
        <ChevronDown className={`h-4 w-4 transition-transform ${advanced ? "rotate-180" : ""}`} />
      </button>
      {advanced ? (
        <div className="grid grid-cols-3 gap-2">
          <div>
            <label className="mb-1 block text-[11px] font-medium uppercase tracking-wide text-muted-foreground">Tempo</label>
